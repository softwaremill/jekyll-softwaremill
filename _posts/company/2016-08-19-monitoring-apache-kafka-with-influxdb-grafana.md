...
* https://sematext.com/spm/integrations/kafka-monitoring/
...
$ curl -POST http://192.168.99.100:8086/query --data-urlencode "q=CREATE DATABASE kafka"
```
...
To expose metrics via remote JMX, a JMX port has to be chosen. This can be achieved by exporting JMX_PORT or starting
Kafka like this:
...
Let's [download the bits](http://central.maven.org/maven2/org/jmxtrans/jmxtrans/) and configure it to gather some
JVM metrics.
...
If we would have [followed the install instructions](https://github.com/jmxtrans/jmxtrans/wiki/Installation), we could
...
Some parameters have been extracted, like URLs and credentials to make it easier to adjust it with command line
...
A corresponding [Grafana Dashboard](https://github.com/softwaremill/confluent-playground/blob/master/metrics/kafka/ApacheKafkaBrokerMetrics.json)
is also available.
...
`-Dcom.sun.management.jmxremote.port=55556 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false`

Then jmxtrans again can be used to read all the required numbers and send them over to InfluxDB.

```
$ JMXTRANS_OPTS="-Dport1=44444 -Durl1=localhost -DinfluxUrl=http://89.75.190.50:8086/ -DinfluxDb=kafka -DinfluxUser=admin -DinfluxPwd=admin" \
...
<img src="/img/kafka-monitoring/onsumer-1.png" />
...
That makes the deployment easier, being just a matter of putting a jar file into Kafka's lib directory and set some
...
about jmxtrans? Should it live together with Kafka on the same box ending up with a jmxtrans process per Kafka node?
